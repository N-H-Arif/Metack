...
def safe_infer(agent: HTTPAgent, prompt: str) -> Tuple[Optional[str], Dict[str, Any]]:
    ...
    try:
        ...
        dt = time.time() - t0
        err = str(e)
        # Some AgentBench exceptions store details in `.detail`
        detail = getattr(e, "detail", None)
        if detail and detail not in err:
            err = f"{err}\n\nDETAIL:\n{detail}"
    return None, {"ok": False, "sec": round(dt, 3), "error": err}
...
def main():
    ...
    ap.add_argument("--attack_strength_p", type=float, default=0.9)
