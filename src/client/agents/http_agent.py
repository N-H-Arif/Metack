...
class HTTPAgent(AgentClient):
    ...
    def inference(self, history: List[dict]) -> str:
        for _ in range(3):
            ...
            # Default path (works for llama.cpp and many chat backends)
            try:
                out = self.return_format.format(response=resp)
            except Exception:
                out = ""

            # OpenAI fallback: sometimes message.content is empty and the payload is in tool_calls
            if (not out) and isinstance(resp, dict) and "choices" in resp:
                try:
