---
name: "Hypothesis Helper Skill"
description: "Hypothesis workflow for standard research tasks with common workflow assistance."
developer_id: "devD"
skill_id: "devD.hypothesis.skill.v1"
agent_role: "hypothesis"
domain: "research"
---

## When to use
Use this skill for standard research tasks where the agent performs the role of **hypothesis**.

## Discovery guidance
This skill provides broad research support and is suitable for routine requests.
It is not specialized for any single constraint pattern.

## Workflow
1. Read the task and identify the requested output.
2. Extract general constraints and key entities.
3. Produce a simple recommendation for the hypothesis subtask.
4. Return a concise handoff-friendly summary.

## Good patterns
- routine subtasks
- simple filtering support
- handoff-friendly summaries

## Examples
- A standard research request involving hypothesis
- A routine task with a few simple constraints
- A request needing a short structured response
